$(document).ready(function() {
  ...
  const cityElem       = $('#city');
  const dateElem       = $('#date');
  const weatherImgElem = $('#weather__img');
  const temp           = $('#temperature');
  const humidity       = $('#humidity');
  const uvIndexElem    = $('#uv__index');
  const windElem       = $('#wind');
  const cityList       = $('.city__list');
  const cityInput      = $('#city__input');
  ...
  $('#search-btn').on('click', function (event) {
      ...
      cityInput.val('');
      if (city) {
          let queryURL = urlFormatFromInputs(city);
  ...
  $('.city-btn').click(function () {
      ...
      let queryURL = urlFormatFromId(foundCity[0].id)
      searchWeather(queryURL);
  });
  ...
  function urlFormatFromInputs(city) {
    if (city) {
        return `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${apiKey}`;
    }
  }

  function urlFormatFromId(id) {
  ...
  function searchWeather(queryURL) {
      $.ajax({
          ...
          cityElem.text(response.name);
          let formattedDate = moment.unix(response.dt).format('L');
          dateElem.text(formattedDate);
          let weatherImg = response.weather[0].icon;
          weatherImgElem.attr('src', `http://openweathermap.org/img/wn/${weatherImg}.png`).attr('alt', response.weather[0].description);
          temp.html(((response.main.temp - 273.15) * 1.8 + 32).toFixed(1));
          humidity.text(response.main.humidity);
          windElem.text((response.wind.speed * 2.237).toFixed(1));
          ...
          $.ajax({
              ...
              uvIndexElem.text(response.current.uvi);
              uvIndexElem.attr('style', `background-color: ${uvColor}; color: ${uvColor === "yellow" ? "black" : "white"}`);
  ...
  function displayLastSearchedCity() {
      if (pastCities[0]) {
          let queryURL = urlFormatFromId(pastCities[0].id);
          searchWeather(queryURL);
      } else {
          let queryURL = urlFormatFromInputs("San Diego");
          searchWeather(queryURL);
      }
  }
